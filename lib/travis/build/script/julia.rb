        ...
        def setup
          ...
          sh.fold 'Julia-install' do
            ...
            when 'linux', 'freebsd'
              ...
              sh.cmd %Q{curl -A "$CURL_USER_AGENT" -S --fail -L --retry 7 '#{julia_url}' } \
                       '| tar -C ~/julia -x -z --strip-components=1 -f -'
              sh.cmd 'export PATH="${PATH}:${TRAVIS_HOME}/julia/bin"'
            when 'osx'
              sh.cmd %Q{curl -A "$CURL_USER_AGENT" -S --fail -L --retry 7 -o julia.dmg '#{julia_url}'}
              ...
              sh.cmd 'export PATH="${PATH}:${TRAVIS_HOME}/julia/bin"'
            when 'windows'
              sh.cmd %Q{curl -A "$CURL_USER_AGENT" -S --fail -L --retry 7 -o julia-installer.exe '#{julia_url}'}

      ...
      class Php < Script
        ...
        def setup
          ...
          if hhvm?
            ...
            sh.mkdir "${TRAVIS_BUILD_HOME}/.phpenv/versions/hhvm/etc/conf.d", recursive: true
          else
            ...
            unless php_5_3_or_older?
              sh.else do
                sh.fold "pearrc" do
                  sh.echo "Writing ${TRAVIS_BUILD_HOME}/.pearrc", ansi: :yellow
...
hhvm.libxml.ext_entity_whitelist=file,http,https
        ...
        def overwrite_pearrc(version)
          ...
          sh.cmd "echo '<?php error_reporting(0); echo serialize(#{pear_config}) ?>' | php > ${TRAVIS_BUILD_HOME}/.pearrc", echo: false
        end
      end
    end
  end

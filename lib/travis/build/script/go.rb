    ...
    class Script
      class Go < Script
        ...
        def prepare
          super
          ensure_gvm_wiped
          sh.if "! -x ${TRAVIS_BUILD_HOME}/bin/gimme" do
            update_gimme
          end
        end
        ...
        def setup
          sh.cmd %Q'GIMME_OUTPUT="$(gimme #{go_version} | tee -a ${TRAVIS_BUILD_HOME}/.bashrc)" && eval "$GIMME_OUTPUT"'
          ...
          # entry.
          sh.export 'GOPATH', "${TRAVIS_BUILD_HOME}/gopath", echo: true
          sh.export 'PATH', "${TRAVIS_BUILD_HOME}/gopath/bin:$PATH", echo: true

          sh.mkdir "${TRAVIS_BUILD_HOME}/gopath/src/#{go_import_path}", recursive: true, assert: false, timing: false
          sh.cmd "rsync -az ${TRAVIS_BUILD_DIR}/ ${TRAVIS_BUILD_HOME}/gopath/src/#{go_import_path}/", assert: false, timing: false

          sh.export "TRAVIS_BUILD_DIR", "${TRAVIS_BUILD_HOME}/gopath/src/#{go_import_path}"
          sh.cd "${TRAVIS_BUILD_HOME}/gopath/src/#{go_import_path}", assert: true
        ...
        private
          ...
          def ensure_gvm_wiped
            ...
            sh.if "-d ${TRAVIS_BUILD_HOME}/.gvm" do
              sh.mv "${TRAVIS_BUILD_HOME}/.gvm", "${TRAVIS_BUILD_HOME}/.gvm.disabled", echo: false
            end
          end

          def install_gimme
            sh.echo "Installing gimme from #{gimme_url.inspect}", ansi: :yellow
            sh.mkdir "${TRAVIS_BUILD_HOME}/bin", echo: false, recursive: true
            sh.cmd "curl -sL -o ${TRAVIS_BUILD_HOME}/bin/gimme '#{gimme_url}'", echo: false
          end
          ...
          def update_gimme
            ...
            sh.mkdir "${TRAVIS_BUILD_HOME}/bin", echo: false, recursive: true
            sh.cmd "curl -sf -o ${TRAVIS_BUILD_HOME}/bin/gimme https://#{app_host}/files/gimme", echo: false
            sh.if "$? -ne 0" do
              install_gimme
            end

            sh.cmd "chmod +x ${TRAVIS_BUILD_HOME}/bin/gimme", echo: false
            sh.export 'PATH', "${TRAVIS_BUILD_HOME}/bin:$PATH", retry: false, echo: false
            # install bootstrap version so that tip/master/whatever can be used immediately
            sh.cmd %Q'gimme #{DEFAULTS[:go]} &>/dev/null'
          end

          def fetch_godep
            godep = "${TRAVIS_BUILD_HOME}/gopath/bin/godep"

            sh.mkdir "${TRAVIS_BUILD_HOME}/gopath/bin", echo: false, recursive: true

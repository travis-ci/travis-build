  ...
  module Build
    class Script
      class Debug
        ...
        attr_accessor :ip_address, :user_name, :password

        def initialize(data)
          @ip_address = data.cache_options.fetch(:ip_address)
          @user_name  = data.cache_options.fetch(:user_name)
          @password   = data.cache_options.fetch(:password)
        end

travis_install_go() {
  ...
  if [[ "${go_version_int}" > "$(travis_vers2int "1.10")" || "${go_version}" == tip || "${go_version}" == master ]] &&
    [[ -f go.mod || "${GO111MODULE}" == on ]]; then
    echo 'Using Go 1.11+ Modules'
  elif [[ "${go_version_int}" > "$(travis_vers2int "1.4")" ]]; then
    ...
    if [[ -f Godeps/Godeps.json ]]; then
      ...
      if [[ "${go_version}" != go1 && "${go_version}" -ge "1.1" ]]; then

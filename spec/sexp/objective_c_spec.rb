...
describe Travis::Build::Script::ObjectiveC, :sexp do
  ...
  describe 'export' do
    it 'sets TRAVIS_XCODE_SDK' do
      ...
      should include_sexp [:export, ['TRAVIS_XCODE_SDK', 'iphonesimulator7.0'], echo: true]
    end

    it 'sets TRAVIS_XCODE_SCHEME' do
      data['config']['xcode_scheme'] = 'MyTests'
      should include_sexp [:export, ['TRAVIS_XCODE_SCHEME', 'MyTests'], echo: true]
    end

    it 'sets TRAVIS_XCODE_PROJECT' do
      data['config']['xcode_project'] = 'MyProject.xcodeproj'
      should include_sexp [:export, ['TRAVIS_XCODE_PROJECT', 'MyProject.xcodeproj'], echo: true]
    end

    it 'sets TRAVIS_XCODE_WORKSPACE' do
      data['config']['xcode_workspace'] = 'MyWorkspace.xcworkspace'
      should include_sexp [:export, ['TRAVIS_XCODE_WORKSPACE', 'MyWorkspace.xcworkspace'], echo: true]
    end
  end

  describe 'install' do
    it 'runs bundle install if the project is a RubyMotion project' do
      sexp = sexp_find(subject, [:if, '-f Gemfile'])
      expect(sexp).to include_sexp [:cmd, 'bundle install', echo: true, timing: true, assert: true, retry: true]
    end

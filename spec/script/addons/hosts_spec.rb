...
describe Travis::Build::Script::Addons::Hosts do
  ...
  it "runs the commands" do
    ...
    script.expects(:cmd).with("sudo #{bin_path}/travis-hosts #{config}", assert: true, echo: false)
    subject
  end
  ...
  context "multiple hostnames" do
    ...
    it "runs the command" do
      script.expects(:fold).with("hosts").yields(script)
      script.expects(:cmd).with("sudo #{bin_path}/travis-hosts johndoe.local example.local", assert: true, echo: false)
      subject
    end
  end
end

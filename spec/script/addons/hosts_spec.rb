...
describe Travis::Build::Script::Addons::Hosts do
  ...
  it "runs the commands" do
    ...
    script.expects(:raw).with("sudo sed -e 's/^\\(127\\.0\\.0\\.1.*\\)$/\\1 'johndoe.local'/' -i'.bak' /etc/hosts")
    script.expects(:raw).with("sudo sed -e 's/^\\(::1.*\\)$/\\1 'johndoe.local'/' -i'.bak' /etc/hosts")
    subject
  end
  ...
  context "multiple hostnames" do
    ...
    it "runs the command" do
      script.expects(:fold).with("hosts").yields(script)
      script.expects(:raw).with("sudo sed -e 's/^\\(127\\.0\\.0\\.1.*\\)$/\\1 'johndoe.local\\ example.local'/' -i'.bak' /etc/hosts")
      script.expects(:raw).with("sudo sed -e 's/^\\(::1.*\\)$/\\1 'johndoe.local\\ example.local'/' -i'.bak' /etc/hosts")

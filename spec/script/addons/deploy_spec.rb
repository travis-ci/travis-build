...
describe Travis::Build::Script::Addons::Deploy do
  ...
  describe 'minimal config' do
    ...
    it 'runs the command' do
      ...
      script.expects(:cmd).with(<<-DPL.gsub(/\s+/, ' ').strip, assert: false, echo: false)
        rvm 1.9.3 do ruby -S dpl --provider="heroku" --password="foo" --email="user@host" --fold;
  ...
  describe 'implicit branches' do
    ...
    it 'runs the command' do
      ...
      script.expects(:cmd).with('rvm 1.9.3 do ruby -S gem install dpl', assert: true, echo: false)
      script.expects(:cmd).with(<<-DPL.gsub(/\s+/, ' ').strip, assert: false, echo: false)
        rvm 1.9.3 do ruby -S dpl --provider="heroku" --app="foo" --fold;
        if [ $? -ne 0 ]; then echo "failed to deploy"; travis_terminate 2; fi
      DPL
      script.expects(:run_stage).with(:after_deploy)
      subject.deploy
  ...
  describe 'on tags' do
    ...
    it 'runs the command' do
      ...
      script.expects(:cmd).with('rvm 1.9.3 do ruby -S gem install dpl', assert: true, echo: false)
      script.expects(:cmd).with(<<-DPL.gsub(/\s+/, ' ').strip, assert: false, echo: false)
        rvm 1.9.3 do ruby -S dpl --provider="heroku" --fold;
        if [ $? -ne 0 ]; then echo "failed to deploy"; travis_terminate 2; fi
      DPL
      script.expects(:run_stage).with(:after_deploy)
      subject.deploy
  ...
  describe 'multiple providers' do
    ...
    it 'runs the command' do
      ...
      script.expects(:cmd).with(<<-DPL.gsub(/\s+/, ' ').strip, assert: false, echo: false)
        rvm 1.9.3 do ruby -S dpl --provider="nodejitsu" --user="foo" --api_key="bar" --fold;
        if [ $? -ne 0 ]; then echo "failed to deploy"; travis_terminate 2; fi
      DPL
      subject.deploy
    end
  ...
  describe 'allow_failure' do
    ...
    it 'runs the command' do
      script.expects(:run_stage).with(:before_deploy)
      script.expects(:if).with('($TRAVIS_PULL_REQUEST = false) && ($TRAVIS_BRANCH = master)').yields(script)
      script.expects(:cmd).with('rvm 1.9.3 do ruby -S gem install dpl', assert: false, echo: false)
      script.expects(:cmd).with(<<-DPL.gsub(/\s+/, ' ').strip, assert: false, echo: false)
        rvm 1.9.3 do ruby -S dpl --provider="heroku" --password="foo" --email="user@host" --fold
      DPL
      script.expects(:run_stage).with(:after_deploy)

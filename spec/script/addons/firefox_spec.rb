...
describe Travis::Build::Script::Addons::Firefox do
  ...
  it 'runs the command' do
    ...
    script.expects(:cmd).with("sudo -n mkdir -p /usr/local/firefox-20.0", assert: false)
    script.expects(:cmd).with("sudo -n chown -R travis /usr/local/firefox-20.0", assert: false)
    ...
    script.expects(:cmd).with("sudo -n ln -sf /usr/local/firefox-20.0/firefox/firefox /usr/local/bin/firefox", assert: false)
    script.expects(:cmd).with("sudo -n ln -sf /usr/local/firefox-20.0/firefox/firefox-bin /usr/local/bin/firefox-bin", assert: false)

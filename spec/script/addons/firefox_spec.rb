...
describe Travis::Build::Script::Addons::Firefox do
  ...
  it 'runs the command' do
    ...
    script.expects(:cmd).with("sudo mkdir -p /usr/local/firefox", assert: false)
    script.expects(:cmd).with("sudo chown travis /usr/local/firefox", assert: false)
    ...
    script.expects(:cmd).with("pushd /usr/local/firefox", assert: false)
    script.expects(:cmd).with("tar xf /tmp/firefox.tar.bz2", assert: false)
    script.expects(:cmd).with("sudo ln -s /usr/local/firefox/firefox /usr/local/bin/firefox", assert: false)

...
describe Travis::Build::Script::Addons::AppEngine do
  ...
  it 'runs the command' do
    ...
    script.expects(:cmd).with('curl -o ~/gae.zip "https://googleappengine.googlecode.com/files/google_appengine_1.8.0.zip"', {:assert => true, :echo => false})
    script.expects(:cmd).with('unzip -q -d ~ ~/gae.zip', {:assert => true, :echo => false})
    ...
    script.expects(:cmd).with('echo foo | ~/google-appengine/appcfg.py update --email=user@host --passin .', {:assert => true, :echo => true})

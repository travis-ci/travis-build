...
describe Travis::Build::Script::Erlang do
  ...
  shared_examples_for 'runs rebar' do |path|
    it "installs #{path}rebar get-deps" do
      is_expected.to run "#{path}rebar get-deps", echo: true, log: true, assert: true, timeout: timeout_for(:install), retry: true
    end
    ...
    it "runs #{path}rebar compile && #{path}rebar skip_deps=true eunit" do
      ...
      is_expected.to run "#{path}rebar skip_deps=true eunit", log: true, timeout: timeout_for(:script)
    end
  end

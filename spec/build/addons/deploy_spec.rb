...
describe Travis::Build::Addons::Deploy, :sexp do
  ...
  describe 'deploy condition fails' do
    ...
    let(:is_pull_request)  { [:echo, 'Skipping deployment with the heroku provider because the current build is a pull request.', ansi: :yellow] }
    let(:not_permitted)    { [:echo, "Skipping deployment with the heroku provider because this branch does not meet a condition", ansi: :yellow] }
    let(:custom_condition) { [:echo, 'Skipping deployment with the heroku provider because a custom condition was not met', ansi: :yellow] }
  ...
  describe 'deploy condition fails with tags' do
    ...
    let(:not_tag) { [:echo, "Skipping deployment with the heroku provider because this is not a tagged commit", ansi: :yellow] }

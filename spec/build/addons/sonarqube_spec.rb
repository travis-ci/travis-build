...
describe Travis::Build::Addons::Sonarqube, :sexp do
  ...
  it_behaves_like 'compiled script' do
    let(:code) { ['curl -sSLo $HOME/.sonarscanner/sonar-scanner.zip', 'echo "export MAVEN_OPTS=\"\$MAVEN_OPTS -Dsonar.host.url=https://sonarqube.com\"" >> ~/.mavenrc'] }
  end
  ...
  describe 'scanner installation' do
    ...
    it { should include_sexp [:export, ['SONAR_SCANNER_OPTS', "\"$SONAR_SCANNER_OPTS -Dsonar.host.url=https://sonarqube.com\""], {:echo=>true}] }
    it { should include_sexp [:export, ['GRADLE_OPTS', "\"$GRADLE_OPTS -Dsonar.host.url=https://sonarqube.com\""], {:echo=>true}] }

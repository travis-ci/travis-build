...
describe Travis::Build::Addons::Sonarqube, :sexp do
  ...
  it_behaves_like 'compiled script' do
    let(:code) { ['curl -sSLo $HOME/.sonarscanner/sonar-scanner.zip https://repox.sonarsource.com/sonarsource-public-releases/org/sonarsource/scanner/cli/sonar-scanner-cli/2.7-build314/sonar-scanner-cli-2.7-build314.zip'] }
  end

  describe 'scanner installation' do
    it { should include_sexp [:export, ['SONAR_SCANNER_HOME', '$HOME/.sonarscanner/sonar-scanner-2.7-build314'], {:echo=>true}] }
    it { should include_sexp [:export, ['PATH', "\"$PATH:$HOME/.sonarscanner/sonar-scanner-2.7-build314/bin\""]] }
  end

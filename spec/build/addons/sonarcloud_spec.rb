...
describe Travis::Build::Addons::Sonarcloud, :sexp do
  ...
  it_behaves_like 'compiled script' do
    let(:code) { ['curl -sSLo "${TRAVIS_BUILD_HOME}/.sonarscanner/sonar-scanner.zip" "http://repo1.maven.org/maven2/org/sonarsource/scanner/cli/sonar-scanner-cli/3.0.3.778/sonar-scanner-cli-3.0.3.778.zip"'] }
  end

  describe 'scanner and build wrapper installation' do
    it { should include_sexp [:export, ['SONAR_SCANNER_HOME', '${TRAVIS_BUILD_HOME}/.sonarscanner/sonar-scanner-3.0.3.778'], {:echo=>true}] }
    it { should include_sexp [:export, ['PATH', "\"$PATH:${TRAVIS_BUILD_HOME}/.sonarscanner/sonar-scanner-3.0.3.778/bin\""]] }

...
describe Travis::Build::Script::DirectoryCache::S3, :sexp do
  ...
  def signed_url_for(branch, signature, ext = 'tbz')
    Shellwords.escape(S3_SIGNED_URL % [url_for(URI::Parser.new.escape(branch), ext), signature])
  end
  ...
  let(:url)           { url_for(URI::Parser.new.escape(branch)) }
  let(:url_tgz)       { url_for(URI::Parser.new.escape(branch), 'tgz') }
  ...
  describe 'fetch' do
    before { cache.fetch }
    it { should include_sexp [:cmd, "rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher --name example cache fetch #{fetch_url_tgz}", timing: true] }
  end

...
describe Travis::Build::Script::DirectoryCache::S3, :sexp do
  ...
  describe 'fetch' do
    before { cache.fetch }
    it { should include_sexp [:cmd, "rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher cache fetch #{fetch_url_tgz}", timing: true] }
  end

  describe 'add' do
    before { cache.add('/foo/bar') }
    it { should include_sexp [:cmd, 'rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher cache add /foo/bar'] }

    context 'when multiple directories are given' do
      before { cache.setup_casher }
      let(:config) { { cache: { directories: ['/foo/bar', '/bar/baz'] } } }

      it { should include_sexp [:cmd, 'rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher cache add /foo/bar /bar/baz'] }
    end
    ...
    context 'when more than ADD_DIR_MAX directories are given' do
      ...
      it { should include_sexp [:cmd, "rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher cache add #{dirs.take(Travis::Build::Script::DirectoryCache::S3::ADD_DIR_MAX).join(' ')}"] }
    end
  end

  describe 'push' do
    before { cache.push }
    it { should include_sexp [:cmd, "rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher cache push #{push_url}", timing: true] }
  end
  ...
  describe 'on a different branch' do
    ...
    describe 'fetch' do
      before { cache.fetch }
      it { should include_sexp [:cmd, "rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher cache fetch #{fetch_url_tgz} #{fallback_url_tgz}", timing: true] }
    end
  ...
  describe 'on a pull request to a different branch' do
    ...
    describe 'fetch' do
      before { cache.fetch }
      it { should include_sexp [:cmd, "rvm $(travis_internal_ruby) --fuzzy do $CASHER_DIR/bin/casher cache fetch #{fetch_url_tgz} #{branch_fallback_url_tgz} #{fallback_url_tgz}", timing: true] }
    end

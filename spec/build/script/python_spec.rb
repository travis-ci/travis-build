...
describe Travis::Build::Script::Python, :sexp do
  ...
  it 'sets TRAVIS_PYTHON_VERSION' do
    should include_sexp [:export,  ['TRAVIS_PYTHON_VERSION', '3.10']]
  end
  ...
  it 'sets up the python version (3.10)' do
    should include_sexp [:cmd,  'source ~/virtualenv/python3.10/bin/activate', assert: true, echo: true, timing: true]
  end
  ...
  context "when python version is given as an array" do
    before { data[:config][:python] = %w(3.10) }
    it 'sets up the python version (3.10)' do
      should include_sexp [:cmd,  'source ~/virtualenv/python3.10/bin/activate', assert: true, echo: true, timing: true]
    end
  end
  ...
  describe 'install' do
    ...
    it 'adds ${TRAVIS_HOME}/.cache/pip to directory cache' do
      should include_sexp [:cmd, %r(rvm \$\(travis_internal_ruby\) --fuzzy do \$CASHER_DIR/bin/casher --name \S+ cache add \${TRAVIS_HOME}/.cache/pip), timing: true]
    end
  end

  it 'sets up python with system site packages enabled' do
    data[:config][:virtualenv] = { 'system_site_packages' => true }
    should include_sexp [:cmd,  'source ~/virtualenv/python3.10_with_system_site_packages/bin/activate', assert: true, echo: true, timing: true]
  end

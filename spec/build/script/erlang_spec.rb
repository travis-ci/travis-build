...
describe Travis::Build::Script::Erlang, :sexp do
  ...
  describe 'setup' do
    it 'activates otp' do
      should include_sexp [:cmd, 'source ${TRAVIS_BUILD_HOME}/otp/R14B04/activate', assert: true, echo: true, timing: true]
    end

    it 'downloads OTP archive on demand when the desired release is not pre-installed' do
      branch = sexp_find(subject, [:if, '! -f ${TRAVIS_BUILD_HOME}/otp/R14B04/activate'])
      ...
      expect(branch).to include_sexp [:cmd, 'wget -o ${TRAVIS_BUILD_HOME}/erlang.tar.bz2 ${archive_url}', assert: true, echo: true, timing: true]
    end
  end

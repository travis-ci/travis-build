      ...
      context "when OTP release is #{otp_release_wanted}" do
        ...
        if otp_release_wanted == otp_release_required
          describe "wanted OTP release #{otp_release_wanted}" do
            it "is installed" do
              sexp = sexp_find(subject, [:if, "! -f ${TRAVIS_BUILD_HOME}/otp/#{otp_release_wanted}/activate"], [:then])
              ...
              expect(sexp).to include_sexp([:cmd, "wget -o ${TRAVIS_BUILD_HOME}/erlang.tar.bz2 ${archive_url}", assert: true, echo: true, timing: true])
            end
          end
        else
          describe "required OTP release #{otp_release_required}" do
            it "is installed" do
              sexp = sexp_find(subject, [:if, "! -f ${TRAVIS_BUILD_HOME}/otp/#{otp_release_required}/activate"], [:then])

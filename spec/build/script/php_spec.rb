...
describe Travis::Build::Script::Php, :sexp do
  ...
  describe 'fixes php.ini for hhvm' do
    ...
    it { should include_sexp [:raw, "grep session.save_path #{path} | cut -d= -f2 | sudo xargs mkdir -m 01733 -p >&/dev/null"] }
  end

   describe 'installs hhvm-nightly' do
    before { data[:config][:php] = 'hhvm-nightly' }
    it { should include_sexp [:cmd, 'travis_apt_get_update'] }
    it { should include_sexp [:cmd, 'sudo apt-get install hhvm-nightly -y 2>&1 >/dev/null', assert: true, timing: true, echo: true] }
   ...
   describe 'installs specific hhvm version' do
    ...
    it { should include_sexp [:cmd, 'travis_apt_get_update'] }
    it { should include_sexp [:cmd, 'sudo apt-get install hhvm -y 2>&1 >/dev/null', timing: true, assert: true, echo: true] }
    it { should include_sexp [:cmd, "echo \"deb [ arch=amd64 ] http://dl.hhvm.com/ubuntu $(lsb_release -sc)-lts-3.12 main\" | sudo tee -a /etc/apt/sources.list >&/dev/null"] }
  end

shared_examples_for 'a script with travis env vars sexp' do
  ...
  it 'sets both global and regular env vars' do
    ...
    should include_sexp [:export, ['FOO', 'foo'], echo: 'export FOO=foo']
    should include_sexp [:export, ['BAR', 'bar'], echo: 'export BAR=bar']
    should include_sexp [:export, ['BAZ', 'baz'], echo: 'export BAZ=baz']
    should include_sexp [:export, ['QUX', 'qux'], echo: 'export QUX=qux']
  end
  ...
  it 'sets environment variables from settings' do
    ...
    should include_sexp [:export, ['SETTINGS_VAR', 'value'], echo: 'export SETTINGS_VAR=[secure]', secure: true]
  ...
  it 'sets environment variables from config' do
    ...
    should include_sexp [:export, ['CONFIG_VAR', 'value'], echo: 'export CONFIG_VAR=[secure]', secure: true]
...
shared_examples_for 'a script with env vars sexp' do
  ...
  it 'sets a given :env var even if empty' do
    ...
    should include_sexp [:export, ['SOMETHING_EMPTY', '""'], echo: 'export SOMETHING_EMPTY=""']
  end

  # TODO what does this add to the tests?
  it 'sets the exact value of a given :env var' do
    data[:config][env_type] = 'BAR=foolish'
    should include_sexp [:export, ['BAR', 'foolish'], echo: 'export BAR=foolish']
  end

  it 'allows setting an env var to another env var' do
    data[:config][env_type] = 'BRANCH=$TRAVIS_BRANCH'
    should include_sexp [:export, ['BRANCH', '$TRAVIS_BRANCH'], echo: 'export BRANCH=$TRAVIS_BRANCH']
  end
  ...
  it 'evaluates and sets the exact values of given :env vars, when their definition is encolsed within single or double quotes' do
    ...
    should include_sexp [:export, ['SINGLE_QUOTED', "'foo+bar (are) on a boat!'"], echo: "export SINGLE_QUOTED='foo+bar (are) on a boat!'"]
    should include_sexp [:export, ['DOUBLE_QUOTED', '"$SINGLE_QUOTED"'], echo: 'export DOUBLE_QUOTED="$SINGLE_QUOTED"' ]
  end
  ...
  it 'sets a given secure :env var and obfuscates it' do
    data[:config][env_type] = 'SECURE BAR=bar'
    should include_sexp [:export, ['BAR', 'bar'], echo: 'export BAR=[secure]', secure: true]
  end

  it 'does not set secure :env vars if they are disabled' do
    data[:job][:secure_env_enabled] = false
    data[:config][env_type] = 'SECURE BAR=bar'
    should_not include_sexp [:export, ['BAR', 'bar'], echo: 'export BAR=[secure]', secure: true]
  end
end

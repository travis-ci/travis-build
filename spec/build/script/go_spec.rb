...
describe Travis::Build::Script::Go, :sexp do
  ...
  it 'sets TRAVIS_GO_VERSION' do
    should include_sexp [:export, ['TRAVIS_GO_VERSION', '1.7.1']]
  end
  ...
  it 'sets the default go version if not :go config given' do
    should include_sexp [:cmd, 'GIMME_OUTPUT=$(gimme 1.7.1) && eval "$GIMME_OUTPUT"', assert: true, echo: true, timing: true]
  end

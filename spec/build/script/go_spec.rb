...
describe Travis::Build::Script::Go, :sexp do
  ...
  shared_examples 'gopath fix' do
    ...
    it { should include_sexp [:cmd, "tar -czf ${TRAVIS_TMPDIR}/src_archive.tar.gz -C ${TRAVIS_BUILD_DIR} . && tar -xzf ${TRAVIS_TMPDIR}/src_archive.tar.gz -C ${TRAVIS_HOME}/gopath/src/#{host}/#{data[:repository][:slug]}", echo: true] }

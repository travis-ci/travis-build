...
describe Travis::Build::Script::Go, :sexp do
  ...
  shared_examples 'gopath fix' do
    ...
    it { should include_sexp [:cmd, "tar -Pczf ${TRAVIS_TMPDIR}/src_archive.tar.gz ${TRAVIS_BUILD_DIR} && tar -Pxzf ${TRAVIS_TMPDIR}/src_archive.tar.gz -C ${TRAVIS_HOME}/gopath/src/#{host}/#{data[:repository][:slug]}", echo: true] }
